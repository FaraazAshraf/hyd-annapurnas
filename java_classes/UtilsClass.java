...
//        int height = displayMetrics.heightPixels;
    ...
    public void checkLocationPermission(final Activity activity) {
        if(ContextCompat.checkSelfPermission(activity, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            ...
            //if user already declined once, it's a good idea to show the rationale behind asking for the permission
            if (ActivityCompat.shouldShowRequestPermissionRationale(activity, Manifest.permission.ACCESS_FINE_LOCATION)) {
                new AlertDialog.Builder(activity)
                        .setTitle("Location Permissions")
                        .setMessage("This feature requires Location Permission. Click on 'Ok' to request permission.")
    ...
    public ArrayList<String> getAllAnnapurnas(Activity activity) {
        InputStreamReader is = null;
        try {
            is = new InputStreamReader(activity.getAssets().open("annapurnas.txt"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        BufferedReader br = new BufferedReader(is);

        final ArrayList<String> allAnnapurnas = new ArrayList<>();
        String line;

        try {
            while((line = br.readLine()) != null) {
                allAnnapurnas.add(line);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return allAnnapurnas;
    }
    ...
    public void onAnnapurnaSelected(Activity activity, String singleAnnapurna) {
        if(new UtilsClass().connectedToInternet(activity)) {
            ...
            goToViewOnMapActivityIntent.putExtra("singleAnnapurna", singleAnnapurna);
